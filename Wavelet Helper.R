...
#This is a repackaging of the dplR continuous wavelet transform and plotting functions.
#I hate the rainbow color palette, so I've replaced it with ColorBrewer.
...
morplot <- function (wave.list, wavelet.levels = quantile(wave.list$Power, 
                                               probs = (0:10)/10), add.coi = TRUE, add.sig = TRUE, x.lab = gettext("Time"), 
          period.lab = gettext("Period"), crn.lab = gettext("Signal"), 
          key.cols = rev(brewer.pal(length(wavelet.levels) - 1, "Spectral")), key.lab = parse(text = paste0("\"", 
